...
/**
 * Provides a friendly event driven API to piond in Node.js. Manages starting and
 * stopping piond as a child process for application support, as well as connecting
 * to multiple piond processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Pion(options) {
  if (!(this instanceof Pion)) {
    return new Pion(options);
  }
...
util.inherits(Pion, Service);

Pion.dependencies = [];

Pion.DEFAULT_MAX_TXIDS = 1000;
Pion.DEFAULT_MAX_HISTORY = 50;
Pion.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Pion.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Pion.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Pion.DEFAULT_SPAWN_RESTART_TIME = 5000;
Pion.DEFAULT_SPAWN_STOP_TIME = 10000;
Pion.DEFAULT_TRY_ALL_INTERVAL = 1000;
Pion.DEFAULT_REINDEX_INTERVAL = 10000;
Pion.DEFAULT_START_RETRY_INTERVAL = 5000;
Pion.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Pion.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Pion.DEFAULT_INSTANTSEND_FEE = 10000;
Pion.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Pion.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Pion.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Pion.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Pion.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Pion.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Pion.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Pion.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Pion.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Pion.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Pion.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Pion.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Pion.prototype._initCaches = function() {
...
Pion.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Pion.prototype.getAPIMethods = function() {
  var methods = [
...
Pion.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'piond/' + name, 'total:', this.subscriptions[name].length);
};

Pion.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'piond/' + name, 'total:', this.subscriptions[name].length);
};

Pion.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (pioncore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'piond/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Pion.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'piond/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Pion.prototype.unsubscribeAddressAll = function(emitter) {
...
Pion.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Pion.DEFAULT_CONFIG_SETTINGS;
...
Pion.prototype._parsePionConf = function(configPath) {
  var options = {};
...
Pion.prototype._expandRelativeDatadir = function() {
...
Pion.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in piond config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in piond config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in piond config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './pion.conf');

  log.info('Using Pion config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parsePionConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parsePionConf(networkConfigPath));
  }
...
Pion.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of pioncore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of pioncore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of pioncore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in pion.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that piond is undergoing a reindex. ' +
             ...
             'of pioncore-node services will start.');
    node._reindex = true;
  }
};

Pion.prototype._resetCaches = function() {
 ...
 * @private
 */
Pion.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Pion.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Pion.prototype._initChain = function(callback) {
  var self = this;
...
Pion.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9953
  };
  if (this.node.network === pioncore.Networks.testnet) {
    networkOptions.rpcport = 19953;
  }
  return networkOptions;
};

Pion.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === pioncore.Networks.testnet) {
    networkPath = 'testnet3/pion.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/pion.conf';
    }
  }
  return networkPath;
};

Pion.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === pioncore.Networks.testnet) {
...
Pion.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('piond/hashblock', message.toString('hex'));
    }
  }

};

Pion.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Pion.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Pion Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Pion.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Pion.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('piond/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Pion.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = pioncore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('piond/rawtransaction', message.toString('hex'));
    }

    var tx = pioncore.Transaction();
    tx.fromString(message);
    var txid = pioncore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Pion.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = pioncore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('piond/transactionlock', message.toString('hex'));
    }

  }
};

Pion.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Pion.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Pion.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Pion.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Pion.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Pion Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Pion.DEFAULT_REINDEX_INTERVAL);

};

Pion.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Pion.prototype._stopSpawnedPion = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/piond.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned pion process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean pion process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Pion.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedPion(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting pion process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Pion process unexpectedly exited with code:', code);
        log.warn('Restarting pion child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Pion process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn piond.'));
      }
...
Pion.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to piond.'));
    }
 ...
 */
Pion.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Pion configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Pion.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Pion.prototype.syncPercentage = function(callback) {
  var self = this;
...
Pion.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, pioncore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Pion.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Pion.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = pioncore.Script.fromAddress(delta.address);
    return {
...
Pion.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Pion.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Pion.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in pioncore is most recent to less recent
 ...
 * @param {Function} callback
 */
Pion.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Pion.prototype._getConfirmationsDetail = function(transaction) {
...
Pion.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Pion.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Pion.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A pion transaction id
 * @param {Function} callback
 */
Pion.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Pion.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof pioncore.Address) {
...
Pion.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Pion.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Pion.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Pion.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Pion.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Pion.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Pioncore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Pion.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = pioncore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Pion.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Pion.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Pion.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Pion.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Pion.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Pion.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Pion.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Pioncore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Pion.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Pion.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
  ...
  function addTxlockToTx(tx,result) {
    tx.txlock = false;
    if (tx.feeSatoshis >= Pion.DEFAULT_INSTANTSEND_FEE) { // if transaction fee is >= 0.0001 check for txlock
      var rawTx = new pioncore.Transaction(result.hex);
      var hash = pioncore.crypto.Hash.sha256sha256(rawTx.toBuffer());
      var id = hash.toString('binary');
      if (self.node.services.piond.zmqKnownTransactionLocks.get(id)) {
 ...
 * @param callback
 */
Pion.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Pion.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Pion.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Pion.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Pion.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Pion.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Pion.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Pion.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Pion.prototype.govCount = function (callback) {
  var self = this;
...
Pion.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
Pion.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Pion.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Pion.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Pion.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Pion.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Pion.prototype.getInfo = function(callback) {
  var self = this;
...
Pion.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Pion.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('piond spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('piond process did not exit'));
      }
