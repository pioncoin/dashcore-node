...
var BASE_PACKAGE = {
  description: 'A full Pion node build with Pioncore',
  repository: 'https://github.com/pioncoin/pioncore-node',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    // 'pioncore-lib': '^' + pioncore.version,
    // 'pioncore-node': version
    "pioncore-lib": "pioncoin/pioncore-lib#master",
    'pioncore-node': 'pioncoin/pioncore-node',
  }
};

/**
 * Will create a directory and pion.conf file for Pion.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createPionDirectory(datadir, done) {
...
/**
 * Will create a base Pioncore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The pion database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/pioncore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Pioncore Node directory, configuration file,
 * pion configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the pioncore node configuration directory
 * @param {String} options.datadir - The path to the pion datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the pioncore-node directory and configuration
    ...
    function(next) {
      // Setup the Pion directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createPionDirectory(absDataDir, next);
      } else {
        next();
      }
    },
