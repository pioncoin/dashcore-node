...
/**
 * Will remove a service from pioncore-node.json
...
/**
 * Will remove the Node.js service and from the pioncore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The pioncore-node configuration directory
...
function remove(options, done) {
  ...
  var pioncoreConfigPath = path.resolve(configPath, 'pioncore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(pioncoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a pioncore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to pioncore-node.json
        removeConfig(pioncoreConfigPath, service, next);
      });
    }, done
  );
}
