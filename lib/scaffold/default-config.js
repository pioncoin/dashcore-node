...
/**
 * Will return the path and default pioncore-node configuration. It will search for the
 * configuration file in the "~/.pioncore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.pioncore');
  var defaultConfigFile = path.resolve(defaultPath, './pioncore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['piond', 'web'];
