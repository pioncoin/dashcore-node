...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for piond is necessary for this upgrade with the "reindex=1" pion.conf option. \n' +
                 'There are changes necessary in both pion.conf and pioncore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/pioncoin/pioncore-node/blob/piond/docs/upgrade.md \n');
        ...
        servicesConfig: {
          piond: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/piond')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed pioncore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './pioncore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in pioncore-node services directory
    ...
    if (servicePackage.pioncoreNode) {
      serviceModule = service.name + '/' + servicePackage.pioncoreNode;
    }
 ...
 * @param {Node} node - The Pioncore Node instance
 */
