...
To develop pioncore-node:

```bash
cd ~
git clone git@github.com:<yourusername>/pioncore-node.git
...
We now will setup symlinks in `pioncore-node` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
...
cd devnode
mkdir node_modules
touch pioncore-node.json
touch package.json
```

Edit `pioncore-node.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../pioncore-node/bin/pioncore-node start
```